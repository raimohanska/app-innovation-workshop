...
Once we click ***Create*** a new API will be added. Inside the new API, go to Settings and uncheck the *Subscription required* and save the changes to make it and open API for all. Now we'll be able to add our single REST operation.
...
We're now ready to create a new API from the Jobs spec found in the Swagger folder. To do this, click on the ***OpenAPI Specification*** options. From the top of the dialot select "Full" to see all the options.
